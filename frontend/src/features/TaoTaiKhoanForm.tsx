...
export function NewAccountForm({ onSave }: Props) {
  ...
  const handleUsernameBlur = async (event: React.FocusEvent<HTMLInputElement>) => {
    ...
    if (username) {
      ...
      try {
        const exists = await kiemTraTenDangNhapTonTai(username);
        ...
        setUsernameAvailable(false);
        setUsernameNotice(
          'Không thể kiểm tra ngay lúc này. Hệ thống sẽ kiểm tra lại khi bạn gửi form đăng ký.',
        );
      }
  ...
  const onSubmit = async (data: TaoTaiKhoanTamThoi) => {
    ...
    if (username) {
      try {
        ...
        console.warn('Khong the kiem tra ten dang nhap truoc khi submit:', error);
        clearErrors('tenDangNhap');
        setUsernameError(null);
        setUsernameNotice(
          'Không thể kiểm tra ngay lúc này. Hệ thống sẽ xác thực lại tên đăng nhập khi gửi form.',
        );
      }
    }
